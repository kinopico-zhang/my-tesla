"""TeslaMate 种子工厂: 充电过程/采样/行程/轨迹点/地址/车辆 的默认行。

默认值沿用旧版替身数据, 断言数字可直接对照; 拆自 conftest.py
(结构化重构: conftest 只留 fixtures, 种子工厂独立成模块供各域测试 import)。
"""
...
from app.tesla.models import (Address, Car, Charge, ChargingProcess, Drive,
                              Position)
